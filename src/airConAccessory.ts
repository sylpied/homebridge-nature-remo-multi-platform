...
export class NatureNemoAirConAccessory {
  ...
  async getTargetTemperature(): Promise<CharacteristicValue> {
    ...
    const airConState = await this.platform.natureRemoApi.getAirConState(this.id);
    const reportedTemperature = parseFloat(airConState.temp);
    if (!Number.isFinite(reportedTemperature) || reportedTemperature < 10 || reportedTemperature > 38) {
      this.platform.logger.debug('[%s] Nature API returned an unavailable target temperature (%s); using %s',
        this.name, airConState.temp, this.state.targetTemperature);
      return this.state.targetTemperature;
    }
    this.platform.logger.info('[%s] Target Temperature -> %s', this.name, reportedTemperature);
    this.state.targetTemperature = reportedTemperature;
